...
// ...............................add Books...................
  ...
  Future<void> addProdects() async {
    String id = DateTime.now().millisecondsSinceEpoch.toString();
    HashMap<String, Object> productDetails = HashMap();
    productDetails["PRODUCT_TITLE"] = titleChangeController.text;
    productDetails["PRODUCT_PRICE"] = changePriceController.text;
    productDetails["CATEGORY_NAME"] = categorynameController.text;
    productDetails["PRODUCT_ID"] = id;
    productDetails["CATEGORY_ID"] = productSelectedcategoryid;
    ...
    // Ensure Firestore update completes
    await db.collection("PRODUCTS").doc(id).set(productDetails);
    GetProdect();
    notifyListeners();
  }
      ...
      if (value.docs.isNotEmpty) {
        ...
        for (var element in value.docs) {
          ...
          ProdectAddList.add(GetProdectDtls(
            element.id,
            map["PRODUCT_TITLE"]?.toString() ?? "",
            map["PHOTO"]?.toString() ?? "",
            map["PRODUCT_PRICE"]?.toString() ?? "",
            map["CATEGORY_NAME"]?.toString() ?? "",
...
//   ........................... filter prodect list............
  ...
  List<GetProdectDtls> filteredProductsList = []; // Filtered products based on category
...
print(filteredProductsList.length);
  ...
  void updateProductList(List<GetProdectDtls> newProductList) {
    ProdectAddList = newProductList;
  filterProductsByCategory(selectedCategory); // Apply filter based on current category
  }
...
// ------------------------------------------------------------------------------
  ...
  String loginUserId = "";
  String loginAddress = "";
  Future<void> usersAuthorized(
      String? lgphoneNumber, String? lgpassword, BuildContext context) async {
    try {
  ...
  void EditCategory(String id) {
    db.collection("CATEGORY").doc(id).get().then((value) {
      ...
      if (value.exists) {
        ...
        // addProductFileImg.text=map["PHOTO"].toString();
        notifyListeners();
      }
    });
    notifyListeners();
